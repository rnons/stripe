...
import           Test.Hspec
import           Test.Hspec.Discover     (print)
...
subscriptionTests stripe = do
  describe "Subscription tests" $ do
    it "Succesfully creates a Subscription" $ do
      ...
      result <- stripe $ do
        ...
        sub <- createSubscription cid
          [CreateSubscriptionSubscriptionItem (PlanId "pid")]
        void $ deletePlan planid
        void $ deleteCustomer cid
        return sub
      result `shouldSatisfy` isRight
    -- xit "Succesfully retrieves a Subscription" $ do
    --   planid <- makePlanId
    --   result <- stripe $ do
    --     Customer { customerId = cid } <- createCustomer
    --     void $ createPlan planid
    ...
    --     return sub
    --   result `shouldSatisfy` isRight
    -- xit "Succesfully retrieves a Subscription expanded" $ do
    --   planid <- makePlanId
    --   result <- stripe $ do
    --     Customer { customerId = cid } <- createCustomer
    --     void $ createPlan planid
    ...
    --     return sub
    --   result `shouldSatisfy` isRight
    -- xit "Succesfully retrieves a Customer's Subscriptions expanded" $ do
    --   planid <- makePlanId
    --   result <- stripe $ do
    --     Customer { customerId = cid } <- createCustomer
    --     void $ createPlan planid
    ...
    --     return sub
    --   result `shouldSatisfy` isRight
    -- xit "Succesfully retrieves a Customer's Subscriptions" $ do
    --   planid <- makePlanId
    --   result <- stripe $ do
    --     Customer { customerId = cid } <- createCustomer
    --     void $ createPlan planid
    ...
    --     return sub
    --   result `shouldSatisfy` isRight
    -- xit "Succesfully retrieves all Subscriptions expanded" $ do
    --   planid <- makePlanId
    --   result <- stripe $ do
    --     Customer { customerId = cid } <- createCustomer
    --     void $ createPlan planid
    ...
    --     return sub
    --   result `shouldSatisfy` isRight
    -- xit "Succesfully retrieves all Subscriptions" $ do
    --   planid <- makePlanId
    --   result <- stripe $ do
    --     Customer { customerId = cid } <- createCustomer
    --     void $ createPlan planid
